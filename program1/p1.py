...
def plotHeatMapForNumberOfPassengers(data, numberOfPassengers, numberOfBucketsPerDim=100):
	...
	heatmap = ax.pcolor(array, cmap=plt.cm.Reds, edgecolors='k')
	#print data
...
def problem1a():
	...
	badDataFilter = derive_BadDataFilter([0, 1, 2])
	compositeFilter = composeFilters([badDataFilter, NYCFilter])
	generatorCreatingFunctionForFilter = createFunctionForGenerator(str_fields, float_fields,row_filter=compositeFilter)
	mean_dev_filter = derive_meanDevFilter(generatorCreatingFunctionForFilter(TRAIN_DATA), [1,2], tolerance=3)

	compositeFilter = composeFilters([badDataFilter, NYCFilter, mean_dev_filter])
	generatorCreatingFunction = createFunctionForGenerator(str_fields, float_fields,row_filter=compositeFilter)
	#training_data = np.array([row for row in generatorCreatingFunction(TRAIN_DATA)])
	training_data = np.array([row for row in generatorCreatingFunction(TRIP_DATA_1)])

	print len(training_data)
	plotHeatMapForNumberOfPassengers(training_data, 1, 25)
	plotHeatMapForNumberOfPassengers(training_data, 3, 25)
...
def problem1d(useStandardization):
	...
	if useStandardization:
		...
		transform = simplePickupTimeTransformer
...
def main():
	...
	problem1b()
	# problem1c()
	#(mean_dev_filter, transform) = problem1d(True)
	#(mean_dev_filter, transform) = problem1d(False)
